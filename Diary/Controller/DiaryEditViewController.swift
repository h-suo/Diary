...
final class DiaryEditViewController: UIViewController, AppResignObservable {
  ...
  override func viewDidLoad() {
    ...
    configureUI()
    setupConstraints()
    addObserveWillResignActive(
      observer: self,
      selector: #selector(endEditingAndPop)
    )
  }
}
...
extension DiaryEditViewController {
  ...
  private func setupNavigationItem() {
    ...
    if diaryEntry == nil {
      let completButton = UIBarButtonItem(
        title: NameSpace.done,
        style: .plain,
        target: self,
        action: #selector(endEditingAndPop)
      )
      
      navigationItem.title = DateFormatManager.dateString(
...
extension DiaryEditViewController: UITextViewDelegate {
  ...
  func textViewDidEndEditing(_ textView: UITextView) {
    let diaryContents = textView
      .text
      .components(separatedBy: NameSpace.enter)
    ...
    guard let locationData,
          let endPoint = CurrentWeatherEndPoint(
